    ...
    <div class="table-box">
      <a-table v-if="role == 1" :columns="columns" :data-source="tableData" :row-key="record => record.id" class="tableLimit" :row-selection="{selectedRowKeys: selectDataId ,onChange: rowSelection}" :bordered="true" :pagination="false" :scroll="{ x: 1500}">
        <span slot="created_at" slot-scope="text, record">
          {{ new Date(record.created_at) | getTime }} -- {{ new Date(record.expire_time) | getTime }}
        </span>
      ...
      <a-table v-else :columns="columns" :data-source="tableData" :row-key="record => record.id" class="tableLimit" :row-selection="{selectedRowKeys: selectDataId ,onChange: rowSelection}" :scroll="{ x: 1500}" :bordered="true" :pagination="false">
      ...
      <!-- 端口续费（临时） -->
      <a-modal v-model="renewDayVisible" title="端口续费(临时)" width="450px" ok-text="确认" cancel-text="取消" @ok="handleRenewDay">
        <a-form-model ref="form" :model="form" label-width="100px" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
          <a-form-item label="唯一串码">
            <a-input v-model="renewDayForm.port_no" disabled />
          </a-form-item>
          <a-form-item label="所属用户ID">
            <a-input v-model="renewDayForm.belong" disabled />
          </a-form-item>
          <a-form-item label="到期时间">
            <a-input-number v-model="renewDayForm.day" style="width:100%" :default-value="30" />
          </a-form-item>
        </a-form-model>
      </a-modal>
      <!-- 批量端口续费 (临时) -->
...
export default {
  ...
  methods: {
    ...
    renewDayDevice(row) {
      ...
      this.renewDayForm.port_no = row.port_no
      this.renewDayForm.belong = row.user_id
      this.renewDayVisible = true
    },
    handleRenewDay() {
      if (this.renewDayForm.day === null || '') {
        this.$message.error('请输入时间')
        return
      }
      let data = {
        list: [this.renewDayForm.id],
        day: this.renewDayForm.day,
        user_id: this.renewDayForm.belong
      }
      api.postAgentPortRenewDay(data).then(res => {
        if (res.code === 0) {
          this.renewDayVisible = false
          this.init()
        }
      })
    },
